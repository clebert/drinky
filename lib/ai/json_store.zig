...
test "File reads keyed entries" {
    ...
    try std.testing.expectEqualStrings(
        "x",
        keyed.entry("openai-plan").?.get("access").?.string,
    );
}
...
test "save replaces the file atomically at owner-only permissions" {
    ...
    try save(gpa, io, path, "openai-plan", entry, .{});
...
test "remove drops only its key on disk, and a missing file opens as null" {
    ...
    try save(gpa, io, path, "openai-plan", entry, .{});
    try save(gpa, io, path, "anthropic-plan", entry, .{});
    try remove(gpa, io, path, "openai-plan");
...
test "a conditional removal keeps a replacement entry" {
    ...
    try save(gpa, io, path, "anthropic-plan", original, .{});
    try save(gpa, io, path, "anthropic-plan", replacement, .{});
...
test "open, save, and remove refuse a corrupt file, leaving it intact on disk" {
    ...
    try std.testing.expectError(
        error.CorruptStore,
        save(gpa, io, path, "openai-plan", entry, .{}),
    );
    try std.testing.expectError(error.CorruptStore, remove(gpa, io, path, "openai-plan"));
...
test "serialize errors on a corrupt file" {
    ...
    try std.testing.expectError(
        error.CorruptStore,
        serialize(gpa, "{ not json", "openai-plan", null, .{}),
    );
}
