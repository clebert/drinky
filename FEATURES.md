...
- When no browser opens, the recorded URL still works, and the callback waits five minutes. When the
  ...
  another path, so Drinky refuses it and the editor keeps the line. A connection that sends no
  request line is dropped at its own deadline, so a stalled browser attempt cannot block the
  callback.
...
- A narrow window shortens the directory, the branch, the context gauge, and the countdowns before
  ...
  the model, and the branch before the directory.
