...
//! page. The title takes the accent role, and every control stays muted.
//!
//! A caption that fits keeps the title and all controls on one row. A caption
//! bounded to one row keeps the title and the longest prefix of whole control
...
pub fn renderRowCells(
    ...
    try attribute.apply(sink, .reset);
    const span = self.rowControlsSpan(columns_max);
    if (span.len > 0) {
...
fn layout(self: *const Caption, columns: usize) Layout {
    ...
    var lines: ControlLines = .{ .rest = self.controls, .columns = @max(columns, 1) };
    var count: usize = 0;
...
fn fitsOneRow(self: *const Caption, columns: usize) bool {
    ...
    const columns_max = @max(columns, 1);
    const title_columns = terminal.width.ofText(self.title);
    if (title_columns > columns_max) return false;
    if (self.controls.len == 0) return true;

    const separator_columns = terminal.width.ofText(paint.separator);
    if (separator_columns > columns_max - title_columns) return false;
    const lead_columns = title_columns + separator_columns;
    return terminal.width.ofText(self.controls) <= columns_max - lead_columns;
}

/// The longest prefix of whole control segments that joins the title on one
/// row of `columns_max`, or empty. Assumes the title fits the row.
fn rowControlsSpan(self: *const Caption, columns_max: usize) []const u8 {
    if (self.controls.len == 0 or
        std.mem.indexOfScalar(u8, self.controls, '\n') != null)
    {
        return "";
    }

    const lead_columns = terminal.width.ofText(self.title) +
        terminal.width.ofText(paint.separator);
    if (lead_columns >= columns_max) return "";
    const room = columns_max - lead_columns;

    var end: usize = 0;
    // `end` grows by at least one separator per pass, so the walk ends at the
    // last segment of the legend.
    while (end < self.controls.len) {
        const start = if (end == 0) 0 else end + paint.separator.len;
        const extended = segmentEnd(self.controls, start);
        if (terminal.width.ofText(self.controls[0..extended]) > room) break;
        end = extended;
    }
    return self.controls[0..end];
}
